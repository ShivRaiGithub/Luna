...
export async function submitTransaction(network: RuntimeNetwork, tx: Record<string, unknown>): Promise<{ txHash?: string; blockHeight?: number }> {
  if (normalizeNetwork(network) === 'undeployed') {
    throw new Error('Sending NIGHT on undeployed is not available from the extension yet. The local node expects a signed extrinsic, so this flow still needs a client-side transaction builder/signer.');
  }
...
export async function requestDust(network: RuntimeNetwork, address: string): Promise<void> {
  ...
  for (const method of attempts) {
    try {
      ...
      // Try the next compatible RPC method.
    }
  }

  if (normalized === 'undeployed') {
    throw new Error('Unable to generate DUST on undeployed from extension RPC alone. Use the local funding flow that registers NIGHT for DUST generation, or wire in the full dust-generation transaction builder.');
  }
